...
def lambda_handler(event, context):
    ...
    if not events:
        logger.info("No events found. Not running EPRTracker script.")
        return {
            "statusCode": 400,
    ...
    for webhook_event in events:
        ...
        if event_type == "updated":
            logger.info(f"EPRTracker update detected. row_id={row_id}, event_id={event_id}")
            should_run = True
            break
    ...
    if should_run:
        ...
        logger.info("No relevant EPRTracker events detected.")

    return {
        "statusCode": 200,
        "body": json.dumps({"message": "EPRTracker webhook received"}),
    }